# nyetdb — the long-form record behind the user guide

This is the prose the user-facing guide in `docs/` was condensed from, kept
verbatim. It is not a second guide: where `docs/GETTING-STARTED.md`,
`docs/COMMANDS.md`, `docs/ENGINES.md` and `docs/SECURITY-MODEL.md` state a
behaviour in one line, this file still carries the paragraph behind it — the
measurement, the alternative that was rejected, and why the refusal is drawn
exactly where it is.

Two reasons it exists rather than living only in the git history:

- **The guide is allowed to be short because this is here.** A fact cut from
  the guide was cut for length, never because it stopped being true. If you
  are about to change a documented behaviour, the argument for the current one
  is in this file — read it before deciding it was arbitrary.
- **It is diffable.** Everything the guide dropped is provably still in the
  repository, so condensing the guide again is a safe operation.

`DEV.md` is the deeper record (module map, test strategy, per-engine denylist
rationale with the measurements, release process) and `DESIGN.md` holds the
decisions and the threat model. This file sits between them and the guide: the
user-visible contract, argued.

Each part below is one document as it stood before the guide was condensed, in
full, with its own heading levels pushed down one.

**Where `DEV.md`, `DESIGN.md` and the `src/` comments say "the README"** — "the
recipe in the README", "documented in the README", "as the README promises" —
they mean this text. It lived in `README.md` until the landing page was cut down
to a page a newcomer would actually read; nothing was dropped on the way, it
moved here.

---

# Part — Configuration (was `docs/CONFIG.md`)
...
# Global audit policy (on by default). See CONTRACT.md, "Audit log".
[audit]
...
# Validator policy tuning — see SECURITY-MODEL.md.
...
# refused outright (NYET / PII_COLUMN, exit 5). See SECURITY-MODEL.md.
[connections.prod.pii]
columns = ["users.email", "users.phone", "customers.ssn"]

# Auto-guardrail: refuse a query whose PLAN is over the limit (see USAGE.md).
...
# see ENGINES.md and SECURITY-MODEL.md.
...
Each engine's own `url`, TLS and connection quirks — and the read-only account
recipe for it — live in [ENGINES.md](../ENGINES.md). The `[pii]` policy is
documented in [SECURITY-MODEL.md](../SECURITY-MODEL.md#pii-columns), the `[audit]`
section in [SECURITY-MODEL.md](../SECURITY-MODEL.md#audit-log), and the `[guardrail]` section
in [COMMANDS.md](../COMMANDS.md#the-auto-guardrail).
...
- **Passwords.** DataGrip keeps them in its own store, and a tool built on
  ...
  line that fills it (see above); a database that authenticates as nobody gets
...
This page is the engine-neutral half. What `nyet query` accepts on MongoDB,
ClickHouse and Redis — and how `schema`, `explain` and `doctor` differ there —
is in [ENGINES.md](../ENGINES.md).
...
- **MySQL/MariaDB** inherits `information_schema`'s own visibility rule, which
  ...
  above applies here too. Grant a genuinely read-only user (see the recipes in
  [ENGINES.md](../ENGINES.md)) if that matters to you.
...
refuses it in **both** modes, `mask` included (see
[PII columns](../SECURITY-MODEL.md#pii-columns)): ask for the columns you want by
name with `nyet query` instead.
...
- **`rows`** is the estimated row count. On PostgreSQL it is the **largest**
  ...
  over-counts sibling correlations (it can refuse one) — see [DEV.md](DEV.md).
...
- **Metadata statements are not estimated.** `SHOW`, `DESCRIBE` and an explicit
  ...
  validator refuses it outright — see `EXPLAIN_ANALYZE` in
  [how to read a refusal](../SECURITY-MODEL.md#how-to-read-a-refusal).)
...
recipes in [ENGINES.md](../ENGINES.md)), not by pointing it at a view.
...
- **read_only_role** (layer 3) — *hybrid*: metadata (superuser? replica?
  ...
  **server** refuses this role's write. If it is refused → `ok` (a direct
  ...
  layer-3 recipes in [ENGINES.md](../ENGINES.md)). Only a *known* server read-only refusal reads as `ok`;
  ...
  `ok` here, so grant your agent's role **only `SELECT`** (the recipes in [ENGINES.md](../ENGINES.md)), not
...
- **ssh_forward** (only with an `[ssh]` section) — what the tunnel left running:
  ...
  is, and the literal `ssh -O exit …` command that removes it. Always `ok` —
...
explain and doctor" in [ENGINES.md](../ENGINES.md#mongodb).
...
resolves it (see [GETTING-STARTED.md](../GETTING-STARTED.md)) — in your editor. The
...
# Part — Engines (was `docs/ENGINES.md`)

## Engines

Six engines, and they are not equally strong. This page says per engine what
`nyet query` accepts, what stands under nyet's own validator (layer 2, the
session read-only), what the guardrail can estimate, and how the `[pii]` policy
behaves — plus the read-only account recipe (layer 3) for each.

| Engine | `nyet query` takes | Layer 2 — session read-only | Guardrail | `[pii]` |
|---|---|---|---|---|
| [PostgreSQL](#postgresql) | SQL | `BEGIN READ ONLY` on a `default_transaction_read_only` connection, plus a server `statement_timeout` | `cost` (default) or `rows` | both nets |
| [MySQL / MariaDB](#mysql--mariadb) | SQL | `START TRANSACTION READ ONLY`, plus `max_execution_time` / `max_statement_time` | `rows` (default) | both nets |
| [SQLite](#sqlite) | SQL | the file is opened `mode=ro` | none — `off` is the only mode | both nets |
| [MongoDB](#mongodb) | a subset of the mongosh read syntax | **none** — MongoDB has no read-only session | none — `off` is the only mode | field names, both nets |
| [ClickHouse](#clickhouse) | SQL | `readonly = 1` on every request — the strongest here | `rows` (default), from `EXPLAIN ESTIMATE` | net B by name only |
| [Redis / Valkey](#redis--valkey) | one command per call | **none** — Redis has no read-only session | none — `off` is the only mode | refused at config parse |

The three layers themselves are described in
[SECURITY-MODEL.md](../SECURITY-MODEL.md); the `[pii]` nets in
[SECURITY-MODEL.md](../SECURITY-MODEL.md#pii-columns).
...
#### Recommended: a read-only role (layer 3)
...
#### Recommended: a read-only user (layer 3)
...
There is no layer 3 here: SQLite has no roles and no server. The file mode is
the whole of it — `nyet doctor` reports the role checks as `na` rather than
inventing a metric.
...
`.distinct(...)`) — see "MongoDB queries" below for exactly what is
...
- **`[pii]` works, but by a different mechanism.** There is no column
  ...
  is returned. See [PII columns](../SECURITY-MODEL.md#pii-columns) for the
  mechanics and the honest cost. The
...
#### Recommended: a read-only account (layer 3)
...
[PII columns](../SECURITY-MODEL.md#pii-columns)).
...
# Part — Security model (was `docs/SECURITY-MODEL.md`)

## Security model

`nyet` enforces read-only in layers, assuming a cooperative but fallible
agent (see the threat model in [DESIGN.md](DESIGN.md)):
...
3. **Read-only database roles** — recommended; makes even direct access
   (bypassing nyet) read-only. See the per-engine recipes in
...
| `EXPENSIVE_QUERY` | the query plan's estimate is above this connection's guardrail limit, so the query was not executed — the envelope carries the plan. Also used when *planning itself* outran the guardrail's budget (then there is no plan to carry): see [the auto-guardrail](../COMMANDS.md#the-auto-guardrail) |
...
| `PII_COLUMN` | the query could expose a column this connection's `[pii]` policy protects — named directly, wrapped in an expression, swept up by `*`, used as a filter, or resolved from the result's provenance. See [PII columns](#pii-columns) |
...
lists (per engine; rationale in [DEV.md](DEV.md)):
...
- **Views are not followed.** The rules apply to the names nyet sees, and on
  ...
  (`columns = ["users.email", "v_users.contact"]`). You do not have to discover
  these by accident: `nyet doctor <alias>` has a **`pii_views`** check that
  names them. On PostgreSQL it walks `pg_depend` for every view and
  materialized view that reads a protected column *and* that this role may
  select from, so revoking `SELECT` on the view settles it as surely as naming
  the view in the policy does. On MongoDB it reads `listCollections` for views
  over a protected collection and then asks each one — with `$type`, which
  answers `"missing"` instead of the value — whether the field actually
  survives its pipeline. A view built the way the `pii_columns` check
  recommends (one that `$unset`s the protected fields) is therefore *not*
  reported: the two checks would otherwise contradict each other. MongoDB has
  no cheap per-object privilege predicate, so that list is "views that expose
  it", with no claim about which of them this role may read — and the check
  says so. **This holds for computed
...
# Part — Output contract (was `docs/CONTRACT.md`)
...
see [Audit log](#audit-log)), `NYET` (with `reason`, see
[SECURITY-MODEL.md](../SECURITY-MODEL.md#how-to-read-a-refusal)),
...
sample of documents — see
[ENGINES.md](../ENGINES.md#mongodb)),
...
[COMMANDS.md](../COMMANDS.md#nyet-sample)),
`PII_MASKED` (`[pii] mode = "mask"`: the named columns came back as
`[REDACTED]` — see [SECURITY-MODEL.md](../SECURITY-MODEL.md#pii-columns)).
